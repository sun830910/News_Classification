...
# load data set
train_df = pd.read_csv('/Users/chiang/数据/天池_零基础入门NLP_新闻文本分类/train_set.csv', sep='\t')
test_df = pd.read_csv('/Users/chiang/数据/天池_零基础入门NLP_新闻文本分类//test_a.csv', sep='\t', nrows=None)
...
for KF_index, (train_index, valid_index) in enumerate(KF.split(X_train)):
    ...
    d_val = xgb.DMatrix(x_valid_, label=y_valid_, reference=d_train)
    clf = xgb.train(params, d_train, num_boost_round=1000, valid_sets=d_val, early_stopping_rounds=100)
    ...
    joblib.dump(clf, f'./Model/xgb_model_5cv_{KF_index}.pkl', compress=3)
